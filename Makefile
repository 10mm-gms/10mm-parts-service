...
container-scan: ## Run container vulnerability scanner
	...
	docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
		...
		aquasec/trivy image --exit-code 1 --severity HIGH,CRITICAL --ignore-unfixed parts-service:latest

test: test-web test-mobile ## Run all unit and integration tests

test-web: ## Run web unit and integration tests
	PYTHONPATH=. $(RUN_UV) pytest src/backend
...
test-e2e: install-e2e ## Run Playwright E2E tests (Mocked)
	...
	rm -f e2e-test.db
	export NVM_DIR="$$HOME/.nvm" && [ -s "$$NVM_DIR/nvm.sh" ] && \. "$$NVM_DIR/nvm.sh" && nvm use --lts && AUTO_SEED_DATA=true MOCK_SES=true ENABLE_TEST_BACKDOOR=true cd src/frontend && npx playwright test
...
test-e2e-system: install-e2e ## Run Playwright E2E tests against real backend
	...
	-pkill -f "vite.*5174" || true
	rm -f e2e-test.db
	export NVM_DIR="$$HOME/.nvm" && [ -s "$$NVM_DIR/nvm.sh" ] && \. "$$NVM_DIR/nvm.sh" && nvm use --lts && PLAYWRIGHT_MODE=SYSTEM AUTO_SEED_DATA=true ENABLE_TEST_BACKDOOR=true cd src/frontend && npx playwright test

test-server: ## Run servers specifically for E2E tests (Template)
	mkdir -p logs
	(PYTHONPATH=src/backend DATABASE_URL=sqlite+aiosqlite:///./e2e-test.db AUTO_SEED_DATA=true TEST_MODE=true ENABLE_TEST_BACKDOOR=true uv run uvicorn main:app --port 8001 > logs/e2e-backend.log 2>&1) & (cd src/frontend && VITE_API_URL=http://localhost:8001 npm run dev -- --port 5174 > ../../logs/e2e-frontend.log 2>&1)

dev: ## Run development servers
	(PYTHONPATH=src/backend uv run uvicorn main:app --reload --port 8001) & (cd src/frontend && npm run dev)
...
build-container: ## Build the web frontend and docker container
	...
	@# Temporarily replace symlink with copy for Docker build context if it's a symlink
	@if [ -L libraries ]; then \
	...
	@if [ -L libraries.tmp ]; then \
		echo "🔗 Restoring libraries symlink..."; \
		rm -rf libraries; \
